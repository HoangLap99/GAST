    ...
    public class HomeController : BasicController
    {
        ...
        public ActionResult Index(object aliasMenuSub, object idSub, object aliasSub, int? page)
        {
            ...
            if (idSub.ToString() != "System.Object")
            {
                ...
                var articlrByRoomRate = db.Articles.Where(a => a.MenuID == menu.ID).ToList();
        ...
        public ActionResult DetailTag(object MenuID, object id, object alias ,int ? page)
        {
            using (var db = new MyDbDataContext())
            {
                Menu menu = db.Menus.FirstOrDefault(a => a.ID == int.Parse(MenuID.ToString()));
                List<Article> listArticle = db.Articles.Where(a => a.Status && a.TagID.Contains(id.ToString()) && a.MenuID==menu.ID).OrderBy(a => a.Createdate).ToList();
                   foreach(Article article in listArticle)
                {
                    article.MenuAlias = menu.Alias;
                }
                int pageNumber = (page ?? 1);
                int pageSize = 9;
                TempData["MenuID"] = int.Parse(MenuID.ToString());

                return View("Article/ListArticle", listArticle.ToPagedList(pageNumber,pageSize));
            }
        }
    }
}